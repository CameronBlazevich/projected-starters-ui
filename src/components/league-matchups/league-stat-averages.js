...
const LeagueStatAverages = (props) => {
    const { teams, currentWeek } = props;
    ...
    const getStatByDisplayName = (team, displayName) => {
        ...
        const displayNamesThatAreAlreadyAvgs = ["ERA", "WHIP", "AVG"];
        if (displayNamesThatAreAlreadyAvgs.includes(displayName)) {
            return stat.value;
        }

        return stat.value / parseInt(currentWeek);
    }
...
export default LeagueStatAverages;