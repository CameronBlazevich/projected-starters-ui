...
const GameSweatContainer = (props) => {
    ...
    const rosteredPlayersInAMatchup = liveGameData.filter(data => mlbIdsForRosteredPlayers.some(p => p == data.matchup.batter.id));

    console.log(rosteredPlayersInAMatchup)

    // ToDo: this should be the filtered by roster ids
    // setShownBatterIds(batterIdsFromEvent);
    const games = rosteredPlayersInAMatchup.map(game => {
        return <GameSweat gameId={game.gameId} matchup={game.matchup} playEvents={game.playEvents} currentHitter={`${game.matchup.batter.id}/${game.matchup.batter.fullName}`}></GameSweat>
    })
